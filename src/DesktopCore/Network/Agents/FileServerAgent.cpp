...
namespace desktop { namespace core { namespace agent {
	...
	, m_server(std::make_unique<httplib::Server>())
	{
		...
		auto host = m_iniFileService->get<std::string>(documents + "Desktop.ini", "FileServer", "Host", "127.0.0.1");
		auto port = m_iniFileService->get<int>(documents + "Desktop.ini", "FileServer", "Port", 9393);
